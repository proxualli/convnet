				...
				while (CurrentEpoch < TotalEpochs)
				{
					...
					if (CheckTaskState())
					{
						...
						if (CheckTaskState())
						{
							...
							SaveWeights((DataProv->StorageDirectory / std::string("definitions") / (Name + std::string("-weights")) / (std::to_string(CurrentCycle) + std::string("_") + std::to_string(CurrentEpoch) + std::string("(") + std::to_string(TestErrors) + std::string(").weights"))).string(), PersistOptimizer);
