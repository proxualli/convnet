...
namespace dnn
{
	...
	public:
		...
		void AddTrainingRate(const TrainingRate rate, const bool clear, const UInt gotoEpoch)
		{
			...
			{
				if ((i + 1) >= gotoEpoch)
					TrainingRates.push_back(TrainingRate(rate.Optimizer, rate.Momentum, rate.L2Penalty, rate.Beta1, rate.Beta2, rate.BatchSize, rate.Cycles, rate.Epochs, rate.EpochMultiplier, newRate, rate.MinimumRate, decayAfterEpochs, Float(1), rate.Dropout, rate.Cutout, rate.AutoAugment, rate.ColorCast, rate.ColorAngle, rate.Distortion, rate.Interpolation, rate.Scaling, rate.Rotation, rate.HorizontalFlip, rate.VerticalFlip));
			...
			if ((totIteration * decayAfterEpochs) < rate.Epochs)
				TrainingRates.push_back(TrainingRate(rate.Optimizer, rate.Momentum, rate.L2Penalty, rate.Beta1, rate.Beta2, rate.BatchSize, rate.Cycles, rate.Epochs - (totIteration * decayAfterEpochs), rate.EpochMultiplier, newRate, rate.MinimumRate, decayAfterEpochs, Float(1), rate.Dropout, rate.Cutout, rate.AutoAugment, rate.ColorCast, rate.ColorAngle, rate.Distortion, rate.Interpolation, rate.Scaling, rate.Rotation, rate.HorizontalFlip, rate.VerticalFlip));
		}
		...
		void AddTrainingRateSGDR(const TrainingRate rate, const bool clear, const UInt gotoEpoch)
		{
			...
			for (auto c = 0ull; c < rate.Cycles; c++)
			{
				...
				for (auto i = 0ull; i < total; i++)
				{
					...
					epoch++;
					if (epoch >= gotoEpoch)
						TrainingRates.push_back(TrainingRate(rate.Optimizer, rate.Momentum, rate.L2Penalty, rate.Beta1, rate.Beta2, rate.BatchSize, c + 1, 1, rate.EpochMultiplier, newRate, minRate, 1, Float(1), rate.Dropout, rate.Cutout, rate.AutoAugment, rate.ColorCast, rate.ColorAngle, rate.Distortion, rate.Interpolation, rate.Scaling, rate.Rotation, rate.HorizontalFlip, rate.VerticalFlip));
