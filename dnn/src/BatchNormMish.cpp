...
namespace dnn
{
	BatchNormMish::BatchNormMish(const dnn::Device& device, const dnnl::memory::format_tag format, const std::string& name, const std::vector<Layer*>& inputs, const bool scaling, const Float momentum, const Float eps, const bool hasBias) :
		Layer(device, format, name, LayerTypes::BatchNormMish, inputs[0]->C, inputs[0]->C, inputs[0]->C, inputs[0]->D, inputs[0]->H, inputs[0]->W, 0, 0, 0, inputs, hasBias),
		Scaling(scaling),
		Eps(eps),
		Momentum(momentum),
	...
	std::string BatchNormMish::GetDescription() const
	{
	...
	size_t BatchNormMish::FanIn() const
	{
		return 1;
	}

	size_t BatchNormMish::FanOut() const
	{
		return 1;
	}

	void BatchNormMish::InitializeDescriptors(const size_t batchSize)
	{
	...
	void BatchNormMish::SetBatchSize(const size_t batchSize)
	{
		Layer::SetBatchSize(batchSize);

		SrcDiff = FloatVector(batchSize * PaddedCDHW, Float(0));
	}

	void BatchNormMish::ForwardProp(const size_t batchSize, const bool training)
	{
					...
					for (auto n = 0ull; n < batchSize; n++)
					{
						...
						for (auto h = 0ull; h < H; h++)
						{
							...
							for (auto w = offsetH; w < offsetH + strideH; w += VectorSize)
								Mish::fVec(mul_add(VecFloat().load_a(&InputLayer->Neurons[w]) - runningMean, weightedInvStdDev, biases)).store_a(&Neurons[w]);
						}
					}
				});
			}
					...
					for (auto n = 0ull; n < batchSize; n++)
					{
						...
						for (auto h = 0ull; h < H; h++)
						{
							...
							for (size_t w = offsetH; w < offsetH + strideH; w += VectorSize)
								Mish::fVec((VecFloat().load_a(&InputLayer->Neurons[w]) - runningMean) * invStdDev).store_a(&Neurons[w]);
						}
					}
				});
			}
				...
				for_i(PaddedC / VectorSize, [=](size_t c)
				{
					...
					for (auto n = 0ull; n < batchSize; n++)
					{
						...
						for (auto h = 0ull; h < H; h++)
						{
							...
							for (auto w = offsetH; w < offsetH + strideH; w += VectorSize)
							{
								Mish::fVec(mul_add(VecFloat().load_a(&InputLayer->Neurons[w]) - mean, weightedInvStdDev, biases)).store_a(&Neurons[w]);
#ifndef DNN_LEAN
				...
				for_i(PaddedC / VectorSize, [=](size_t c)
				{
					...
					for (auto n = 0ull; n < batchSize; n++)
					{
						...
						for (auto h = 0ull; h < H; h++)
						{
							...
							for (auto w = offsetH; w < offsetH + strideH; w += VectorSize)
							{
								Mish::fVec((VecFloat().load_a(&InputLayer->Neurons[w]) - mean) * invStdDev).store_a(&Neurons[w]);
#ifndef DNN_LEAN
								vecZero.store_nt(&NeuronsD1[w]);
#endif
							}
	...
	void BatchNormMish::BackwardProp(const size_t batchSize)
	{
#ifdef DNN_LEAN
		...
		for_i(PaddedC / VectorSize, [=](size_t c)
		{
			...
			for (auto n = 0ull; n < batchSize; ++n)
			{
				...
				for (auto h = 0ull; h < H; ++h)
				{
					...
					for (auto w = offsetH; w < offsetH + strideH; w += VectorSize)
					{
						diffSrc = Mish::dfVec(VecFloat().load_a(&Neurons[w])) * VecFloat().load_a(&NeuronsD1[w]);
			...
			for (auto n = 0ull; n < batchSize; ++n)
			{
				...
				for (auto h = 0ull; h < H; ++h)
				{
					...
					for (auto w = offsetH; w < offsetH + strideH; w += VectorSize)
					{
						diffSrc = mul_add(Mish::dfVec(VecFloat().load_a(&Neurons[w])), VecFloat().load_a(&NeuronsD1[w]), -mul_add(VecFloat().load_a(&InputLayer->Neurons[w]) - mean, diffGamma, diffBeta));
	...
	ByteVector BatchNormMish::GetImage(const Byte fillColor)
	{
		if (Scaling)
		{
