...
namespace dnn
{
	BatchNormMishDropout::BatchNormMishDropout(const dnn::Device& device, const dnnl::memory::format_tag format, const std::string& name, const std::vector<Layer*>& inputs, const Float dropout, const bool scaling, const Float momentum, const Float eps, const bool hasBias) :
		Layer(device, format, name, LayerTypes::BatchNormMishDropout, inputs[0]->C, inputs[0]->C, inputs[0]->C, inputs[0]->D, inputs[0]->H, inputs[0]->W, 0, 0, 0, inputs, hasBias),
		Scaling(scaling),
		Eps(eps),
		Momentum(momentum),
	...
	std::string BatchNormMishDropout::GetDescription() const
	{
	...
	size_t BatchNormMishDropout::FanIn() const
	{
		return 1;
	}

	size_t BatchNormMishDropout::FanOut() const
	{
		return 1;
	}

	void BatchNormMishDropout::InitializeDescriptors(const size_t batchSize)
	{
	...
	void BatchNormMishDropout::SetBatchSize(const size_t batchSize)
	{
	...
	void BatchNormMishDropout::ForwardProp(const size_t batchSize, const bool training)
	{
					...
					for (auto n = 0ull; n < batchSize; n++)
					{
						...
						for (auto h = 0ull; h < H; h++)
						{
							...
							for (auto w = offsetH; w < offsetH + strideH; w += VectorSize)
							{
								...
								(neuronsActive * Scale * Mish::fVec(mul_add(VecFloat().load_a(&InputLayer->Neurons[w]) - mean, weightedInvStdDev, biases))).store_a(&Neurons[w]);
#ifndef DNN_LEAN
					...
					for (auto n = 0ull; n < batchSize; n++)
					{
						...
						for (auto h = 0ull; h < H; h++)
						{
							...
							for (auto w = offsetH; w < offsetH + strideH; w += VectorSize)
							{
								...
								(neuronsActive * Scale * Mish::fVec((VecFloat().load_a(&InputLayer->Neurons[w]) - mean) * invStdDev)).store_a(&Neurons[w]);
#ifndef DNN_LEAN
								vecZero.store_nt(&NeuronsD1[w]);
#endif
							}
	...
	void BatchNormMishDropout::BackwardProp(const size_t batchSize)
	{
#ifdef DNN_LEAN
		...
		for_i(PaddedC / VectorSize, [=](size_t c)
		{
			...
			for (size_t n = 0; n < batchSize; ++n)
			{
				...
				for (auto h = 0ull; h < H; ++h)
				{
					...
					for (auto w = offsetH; w < offsetH + strideH; w += VectorSize)
					{
						diffSrc = VecFloat().load_a(&NeuronsActive[w]) * Mish::dfVec(VecFloat().load_a(&Neurons[w])) * VecFloat().load_a(&NeuronsD1[w]);
	...
	ByteVector BatchNormMishDropout::GetImage(const Byte fillColor)
	{
		if (Scaling)
		{
