...
namespace dnn
{
	BatchNormHardSwishDropout::BatchNormHardSwishDropout(const dnn::Device& device, const dnnl::memory::format_tag format, const std::string& name, const std::vector<Layer*>& inputs, const Float dropout, const bool scaling, const Float momentum, const Float eps, const bool hasBias) :
	...
	std::string BatchNormHardSwishDropout::GetDescription() const
	{
	...
	size_t BatchNormHardSwishDropout::FanIn() const
	{
		return 1;
	}

	size_t BatchNormHardSwishDropout::FanOut() const
	{
		return 1;
	}

	void BatchNormHardSwishDropout::InitializeDescriptors(const size_t batchSize)
	{
	...
	void BatchNormHardSwishDropout::SetBatchSize(const size_t batchSize)
	{
	...
	void BatchNormHardSwishDropout::ForwardProp(const size_t batchSize, const bool training)
	{
	...
	void BatchNormHardSwishDropout::BackwardProp(const size_t batchSize)
	{
#ifdef DNN_LEAN
	...
	ByteVector BatchNormHardSwishDropout::GetImage(const Byte fillColor)
	{
		if (Scaling)
		{
