...
{
	if (model)
		model->AddTrainingRate(TrainingRate(optimizer, momentum, L2penalty, beta1, beta2, batchSize, cycles, epochs, epochMultiplier, maximumRate, minimumRate, decayAfterEpochs, decayFactor, dropout, cutout, autoAugment, colorCast, colorAngle, distortion, interpolation, scaling, rotation, horizontalFlip, verticalFlip), clear, gotoEpoch);
}
...
{
	if (model)
		model->AddTrainingRateSGDR(TrainingRate(optimizer, momentum, L2penalty, beta1, beta2, batchSize, cycles, epochs, epochMultiplier, maximumRate, minimumRate, decayAfterEpochs, decayFactor, dropout, cutout, autoAugment, colorCast, colorAngle, distortion, interpolation, saling, rotation, horizontalFlip, verticalFlip), clear, gotoEpoch);
}
